	...
	: public operator_base<rxu::value_type_t<rxu::decay_t<T>>>
{
	...
	merge_error_delay(const source_type& o, coordination_type sf)
		: initial(o.source_operator, std::move(sf))
	{
	}
	...
	void on_subscribe(Subscriber scbr) const {
			...
			, public values
		{
			...
			// subscriptions have received on_completed
			int pendingCompletions;
			std::vector<std::exception_ptr> lastExceptions;
			coordinator_type coordinator;
			output_type out;
		};
		...
		// on_next
			[state](source_value_type st) {
				...
				//on_completed
					[state](){
						if (--state->pendingCompletions == 0) {
							if(!state->lastExceptions.empty()) {
								for(std::exception_ptr e : state->lastExceptions) {
									state->out.on_error(e);
								}
							} else {
								state->out.on_completed();
...
template<class Coordination>
class merge_error_delay_factory
{
	typedef rxu::decay_t<Coordination> coordination_type;

	coordination_type coordination;
public:
	merge_error_delay_factory(coordination_type sf)
		: coordination(std::move(sf))
	{
	}
	...
	template<class Observable>
	auto operator()(Observable source)
		->      observable<rxu::value_type_t<merge_error_delay<rxu::value_type_t<Observable>, Observable, Coordination>>,   merge_error_delay<rxu::value_type_t<Observable>, Observable, Coordination>> {
		return  observable<rxu::value_type_t<merge_error_delay<rxu::value_type_t<Observable>, Observable, Coordination>>,   merge_error_delay<rxu::value_type_t<Observable>, Observable, Coordination>>(
																												merge_error_delay<rxu::value_type_t<Observable>, Observable, Coordination>(std::move(source), coordination));
	}
};
...
inline auto merge_error_delay()
	->      detail::merge_error_delay_factory<identity_one_worker> {
	return  detail::merge_error_delay_factory<identity_one_worker>(identity_current_thread());
}

template<class Coordination>
auto merge_error_delay(Coordination&& sf)
	->      detail::merge_error_delay_factory<Coordination> {
	return  detail::merge_error_delay_factory<Coordination>(std::forward<Coordination>(sf));
}
